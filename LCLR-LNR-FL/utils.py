...
def process_control():
    ...
    if cfg['data_name'] in ['MNIST']:
        ...
        if cfg['data_split_mode'] == 'iid':
            cfg['num_epochs'] = {'global': 1, 'local': 5}  # 200 5
            cfg['batch_size'] = {'train': 10, 'test': 50}  # 10
            cfg['milestones'] = [100]
        ...
        # cfg['optimizer_name'] = 'SGD' # 'SGD'
        # cfg['lr'] = 0.05 # 1e-1
        cfg['optimizer_name'] = 'Adam'
        # cfg['lr'] = 1e-3
        cfg['lr'] = 1e-3  # accelerate for local debug
        cfg['momentum'] = 0.9
        cfg['weight_decay'] = 5e-4
        cfg['scheduler_name'] = 'MultiStepLR'
        cfg['factor'] = 0.1
        if cfg['data_split_mode'] == 'iid':
            cfg['num_epochs'] = {'global': 20, 'local': 5}  # global：400 5
            cfg['batch_size'] = {'train': 128, 'test': 50}
