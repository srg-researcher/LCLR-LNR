...
class Logger():
    ...
    def append(self, result, tag, n=1, mean=True):
        for k in result:
            ...
            if mean:
                if isinstance(result[k], Number):  # check if an object is an instance of a particular class or type
                    self.counter[name] += n
                    self.mean[name] = ((self.counter[name] - n) * self.mean[name] + n * result[k]) / self.counter[name]  # 为什么我觉得这个地方算的有点复杂了呢？是我理解的不对吗？
