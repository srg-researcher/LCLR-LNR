    ...
    def __init__(self, dataset, sample_ratio, r, noise_mode, root_dir, transform, mode, noise_file='', pred=[], probability=[], log=''): 
        ...
        if self.mode=='test':
            ...
            if os.path.exists(noise_file):             
                ...
                np.savez('noisy_indices_alldata.npz', noisy_indices=noise_idx)
                ...
                np.savez(noise_file, label = noise_label, index = noise_idx, clean_idx = clean_idx_in_noises)
                for kk in range(num_class):
            ...
            elif self.mode == 'aw_sampling':  # added by ying for our sampling model 2023-11-14
                # we save the selected index into the pred list
                pred_idx = pred
                
                self.train_data = train_data[pred_idx]
                self.noise_label = [noise_label[i] for i in pred_idx]

            elif self.mode == 'aw_labeled' or self.mode == 'aw_unlabeled':
                if self.mode == "aw_labeled":
                    pred_idx = pred
                elif self.mode == "aw_unlabeled":
                    idx = list(range(num_sample))
                    pred_idx_noisy = [x for x in idx if x not in pred]
                    pred_idx = pred_idx_noisy

                self.train_data = train_data[pred_idx]
                self.noise_label = [noise_label[i] for i in pred_idx]

            elif self.mode == 'aw_sampling_bal':
                save_file = 'Clean_asym_index_onlybalance_' + str(dataset) + '_' + str(noise_mode) + '_' + str(self.r) + '.npz'
...
class cifar_dataloader():  
    ...
    def run(self, sample_ratio, mode, pred=[], prob=[]):
        if mode=='warmup':
            ...
            all_dataset = cifar_dataset(dataset=self.dataset, sample_ratio= sample_ratio, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=self.transforms["aw_sampling"], mode="all",noise_file=self.noise_file,  pred=pred)
            trainloader = DataLoader(
                dataset=all_dataset,
                batch_size=self.batch_size*2,
                shuffle=True,
        ...
        elif mode=='aw_sampling':
            all_dataset = cifar_dataset(dataset=self.dataset, sample_ratio= sample_ratio, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=self.transforms["aw_sampling"], mode="aw_sampling",noise_file=self.noise_file,  pred=pred)
            trainloader = DataLoader(
                dataset=all_dataset,
                batch_size=self.batch_size*2,
                shuffle=True,
        ...
        elif mode=='aw_sampling_augmentation':
            all_dataset = cifar_dataset(dataset=self.dataset, sample_ratio= sample_ratio, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=self.transforms["labeled"], mode="aw_sampling",noise_file=self.noise_file,  pred=pred)
            trainloader = DataLoader(
                dataset=all_dataset,
            ...
            labeled_dataset = cifar_dataset(dataset=self.dataset, sample_ratio=sample_ratio, noise_mode=self.noise_mode,
                                            ...
                                            mode="aw_labeled", noise_file=self.noise_file, pred=pred)
            ...
            unlabeled_dataset = cifar_dataset(dataset=self.dataset, sample_ratio=sample_ratio,
                                              ...
                                              transform=self.transforms["unlabeled"], mode="aw_unlabeled",
        ...
        elif mode=='aw_sampling_bal':
            all_dataset = cifar_dataset(dataset=self.dataset, sample_ratio= sample_ratio, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=self.transforms["aw_sampling"], mode="aw_sampling_bal",noise_file=self.noise_file,  pred=pred, probability=prob)
            trainloader = DataLoader(
                dataset=all_dataset,
                batch_size=self.batch_size,
                shuffle=True,
